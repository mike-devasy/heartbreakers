              ...
              if (templateConfig.images.optimize.enable) {
                ...
                for (const file of files) {
                  ...
                  if (file.endsWith(".html") || file.endsWith(".js")) {
                    ...
                    content = await replaceAsync(
                      content,
                      /url\(['"]?(https?:\/\/[^\s'"]+\.(?:jpg|jpeg|png|gif|webp|avif)|[^\s'"]+\.(?:jpg|jpeg|png|gif|webp|avif))['"]?\)/gi,
                      async (data, url) => {
                        const cssIgnore =
                          templateConfig.images.optimize.cssignore || []
                        ...
                        const isIgnored = cssIgnore.some((item) => {
                          const cleanItem = normalizePath(item)
                            .replace(/^(\.\/|\.\.\/)+/, "")
                            .replace(/^assets\/img\//, "")
                            .replace(/^img\//, "")

                          return cleanUrl.endsWith(cleanItem)
                        })
...
async function imageResizeInit(
  ...
  if (mode === "html" || mode === "js") {
    ...
    if (templateConfig.images.optimize.modernformat.only || isWebpAvif) {
      ...
      !isWebpAvif
        ? (templete += `<source srcset="${dpiImages ? dpiImages : imageOptimazeUrl}" type="image/${imageoutExt}">`)
        : null
